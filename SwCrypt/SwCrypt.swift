...
open class PEM {
	...
	open class EncryptedPrivateKey {
		...
		fileprivate static func getIV(_ strippedKey: String) -> Data? {
        	let ivInHex = String(strippedKey[strippedKey.index(strippedKey.startIndex, offsetBy:aesInfoLength)
                ..< strippedKey.index(strippedKey.startIndex, offsetBy:aesHeaderLength)])
